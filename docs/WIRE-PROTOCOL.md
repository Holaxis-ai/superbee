...
| WIRE-PROOF-08 | Remote canonical export differs from an original-byte guarantee. | `packages/cli-runtime/src/commands/doc/common.ts::canonical OKF re-serialization` | `packages/cli-runtime/test/remote.test.ts::canonical re-serialization is byte-identical` |
| WIRE-PROOF-09 | Missing version transport fails closed. | `packages/core/src/remote-backend.ts::VERSION_MISSING` | `packages/cli-runtime/test/remote-auth.test.ts::response stripped of BOTH version headers` |
