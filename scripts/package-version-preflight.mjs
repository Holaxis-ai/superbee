...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  if (process.argv.length !== 2) {
    console.error('Usage: node scripts/package-version-preflight.mjs (no arguments; checks this repository)');
    process.exit(2);
  }
  const start = performance.now();
  const errors = checkPackageVersions();
  if (errors.length) { console.error(errors.join('\n')); process.exitCode = 1; }
  else console.log(`Package version source preflight passed (${Math.round(performance.now() - start)} ms; no install/build/registry access).`);
}
