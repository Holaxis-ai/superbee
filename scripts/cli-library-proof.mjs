...
// This proof only consumes retained bytes. Development tests own their build/pack setup.
export async function proveCliTarball(tarball, { checkSource, execute = exec } = {}) {
  ...
  try {
    ...
    assert.ok(stat.isFile() && !stat.isSymbolicLink(), "retained artifact must be a regular file");
    const before = createHash("sha256").update(await readFile(tarball)).digest("hex");
    const listing = (await exec("tar", ["-tzf", tarball])).stdout.trim().split("\n");
    assert.ok(listing.every(file => ["package/package.json", "package/README.md"].includes(file) || /^package\/dist\/(?!.*(?:^|\/)\.\.(?:\/|$))/.test(file)));
    const lock = JSON.parse(await readFile(path.join(root, "package-lock.json"), "utf8"));
    await writeFile(path.join(scratch, "package.json"), JSON.stringify({ private: true, type: "module", devDependencies: { "@types/node": lock.packages["node_modules/@types/node"].version }, dependencies: { "@superbee/cli": `file:${tarball}` } }));
...
`);
    ...
    assert.equal(createHash("sha256").update(await readFile(tarball)).digest("hex"), before, "retained tarball changed during proof");
