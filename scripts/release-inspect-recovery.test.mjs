...
test("approval inspection verifies the stable successor package coordinate from candidate target metadata", async () => {
  const h = scratch();
  try {
    const signer = makeSigner(h.root, "briand-ai");
    const allowedPath = path.join(h.root, "allowed-signers");
    ...
    const candidateBytes = Buffer.from(JSON.stringify({
      ...
      tag: "v0.1.0",
      version: "0.1.0",
      tarball: { version: "0.1.0", sha256: tarballSha, integrity: "sha512-c3VwZXJiZWU=" },
    }));
    const candidate = { id: 22, name: "candidate.json", digest: digest(candidateBytes), uploader: { login: "github-actions[bot]" } };
    const commands = [];
    const requests = [];
    ...
    function run(command, args, options = {}) {
      ...
      if (command === "gh" && args[0] === "api" && args.includes("repos/Holaxis-ai/superbee-rename-mirror/releases/300")) {
        return JSON.stringify({
          id: 300, draft: true, tag_name: "v0.1.0",
      ...
      if (command === "npm" && args[0] === "view") {
        assert.deepEqual(args, ["view", "superbee@0.1.0", "dist", "--json"]);
    ...
    assert.ok(commands.some((item) => item.command === "npm" && item.args[1] === "superbee@0.1.0"));
