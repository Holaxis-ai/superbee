...
import { isStrictSemver, parseStrictSemver } from "./strict-semver.mjs";
...
export const PUBLISHED_PUBLICATION_PROOF_SCHEMA = "superbee.published-publication-proof.v1";
...
export function releaseTier(version) {
  const parsed = parseStrictSemver(strictVersion("version", version));
...
function verifyPublicationInventory({ release, plan, label }) {
  if (plan?.schema !== PUBLICATION_PLAN_SCHEMA) fail("unknown publication plan schema");
  const expected = proofMap([...plan.keep.core_assets, ...plan.keep.receipt_assets], `${label} keep asset`);
  const observedNames = new Set();
  let statusAsset = null;
  for (const raw of release?.assets ?? []) {
    const asset = exactAssetTriple(raw, `${label} asset`);
    if (observedNames.has(asset.name)) fail(`duplicate ${label} asset ${asset.name}`);
    observedNames.add(asset.name);
    const fixed = expected.map.get(asset.name);
    if (fixed) {
      if (!sameTriple(asset, fixed)) fail(`${label} asset ${asset.name} differs from verified id/name/digest`);
      continue;
    }
    if (plan.keep.status && asset.name === plan.keep.status.name) {
      if (asset.digest !== plan.keep.status.digest) fail(`${label} status asset ${asset.name} differs from generated bytes`);
      statusAsset = asset;
      continue;
    }
    fail(`unexpected ${label} asset ${asset.name}`);
  }
  for (const name of expected.map.keys()) if (!observedNames.has(name)) fail(`${label} inventory is missing ${name}`);
  if (plan.keep.status && !statusAsset) fail(`${label} inventory is missing generated status ${plan.keep.status.name}`);
  if (!plan.keep.status && statusAsset) fail(`${label} inventory retains an unexpected status asset`);
...
export function verifyFinalPublication({ release, plan }) {
  ...
  const { assets, statusAsset } = verifyPublicationInventory({ release, plan, label: "final" });
  return {
...
/**
 * Prove the observable GitHub publication state after PATCH. The manifest-derived tuple is passed
 * by the workflow adapter that loaded it once; chain and plan remain the independent identity and
 * byte-inventory authorities.
 */
export function verifyPublishedPublication({ release, latestRelease, plan, chain, tuple }) {
  if (chain?.schema !== RELEASE_FINALIZER_PROOF_SCHEMA) fail("unknown finalizer chain proof schema");
  if (!tuple || typeof tuple !== "object") fail("missing manifest publication tuple");
  if (tuple.target !== chain.target || tuple.version !== chain.version || tuple.tag !== chain.tag) {
    fail("manifest tuple differs from the verified finalizer chain");
  }
  if (plan?.schema !== PUBLICATION_PLAN_SCHEMA || plan.tag !== tuple.tag) fail("publication plan tag differs from manifest tuple");
  const releaseId = numericAssetId("published release id", release?.id);
  if (
    releaseId !== numericAssetId("chain draft release id", chain.draft_release_id)
    || releaseId !== plan.draft_release_id
  ) {
    fail("published release id differs from the verified numeric release id");
  }
  if (release?.draft !== false) fail("published release remains a draft");
  if (release?.tag_name !== tuple.tag) fail(`published tag ${JSON.stringify(release?.tag_name)} != ${tuple.tag}`);
  field("chain source commit", chain.source_commit, /^[a-f0-9]{40}$/);
  if (release?.target_commitish !== chain.source_commit) fail("published target commit differs from verified source commit");
  const prerelease = releaseTier(tuple.version) === "prerelease";
  if (release?.prerelease !== prerelease) fail(`published prerelease flag ${JSON.stringify(release?.prerelease)} != ${prerelease}`);
  if (latestRelease !== null && (!latestRelease || typeof latestRelease !== "object" || Array.isArray(latestRelease))) {
    fail("latest release observation must be an object or null");
  }
  const latestReleaseId = latestRelease === null ? null : numericAssetId("latest release id", latestRelease.id);
  const githubLatest = tuple.publication?.github_latest;
  if (typeof githubLatest !== "boolean") fail("manifest tuple has invalid GitHub latest policy");
  if (githubLatest && latestReleaseId !== releaseId) fail("published release is not the GitHub latest release");
  if (!githubLatest && latestReleaseId === releaseId) fail("non-latest publication became the GitHub latest release");
  const { assets, statusAsset } = verifyPublicationInventory({ release, plan, label: "published" });
  return {
    schema: PUBLISHED_PUBLICATION_PROOF_SCHEMA,
