...
VARIABLES
  ...
  createdAt, firstSend, lastSend, lastClaim, sendClaim,
  \* --- ghosts and budgets
...
vars == <<head, hseq, ownSeq, applied, rec, horizon, reqs, resps, cst, rid, base, bseq,
          claimed, subs, createdAt, firstSend, lastSend, lastClaim, sendClaim,
          lostUpdate, misleading, badAck, clock, losses, tps, restarts, auths, keeps, resumes>>
...
Init ==
  ...
  /\ createdAt = 0 /\ firstSend = NoTime /\ lastSend = NoTime /\ lastClaim = NoTime /\ sendClaim = NoTime
...
SendSubmit(claimT) ==   \* uncertain-write.ts:269-275 (subs < maxSubmissions checked by caller)
...
Push ==
  ...
  /\ \/ /\ ~claimed
     ...
     \/ /\ ~claimed                 \* hosted: refused AUTH_REQUIRED, "not sent"; the claim
        ...
        /\ claimed' = TRUE
        /\ cst' = "refused"
        /\ UNCHANGED <<reqs, resps, subs, firstSend, lastSend, sendClaim>>
     \/ /\ claimed
        /\ subs' = 0
        /\ SendLookup
        /\ UNCHANGED <<claimed, firstSend, lastSend, sendClaim, auths, resps>>
...
Timeout ==
  ...
  /\ UNCHANGED <<head, hseq, ownSeq, applied, rec, horizon, rid, base, bseq, claimed, subs,
                 createdAt, firstSend, lastSend, lastClaim, sendClaim, lostUpdate, misleading, badAck,
...
Recv ==
  ...
  /\ \E a \in resps :
       ...
       /\ CASE a.t = "rec" ->                               \* committed/conflict answer
                 ...
                 /\ UNCHANGED <<reqs, subs, claimed, firstSend, lastSend, sendClaim>>
            [] a.t = "stale" ->                             \* fixed+server guard: the store
                 /\ ClientDone(IF a.head = Local THEN "ack" ELSE "unknownReview", a.head)
                 /\ UNCHANGED <<reqs, subs, claimed, firstSend, lastSend, sendClaim>>  \* refused a key it cannot vouch for
            ...
            [] a.t = "absent" /\ cst = "look" ->            \* 404 / hosted "absent"
                 LET null == CASE Mode = "ref"    -> TRUE
                               ...
                               [] Mode = "hostedFix" -> HostedTrusts
                               [] Mode = "fixed"  -> FixedCovered
                 IN IF null
                    THEN IF subs < MaxSubs                  \* uncertain-write.ts:290-291,269-270
                         THEN /\ SendSubmit(lastClaim) /\ misleading' = misleading /\ badAck' = badAck
...
Keep ==
  ...
  /\ base' = head /\ bseq' = hseq
  /\ claimed' = FALSE /\ subs' = 0
  /\ createdAt' = clock /\ firstSend' = NoTime /\ lastSend' = NoTime /\ lastClaim' = NoTime /\ sendClaim' = NoTime
  /\ cst' = "pending"
...
DeliverSub(m) ==
  ...
  /\ IF Live(m.rid)
     ...
     ELSE IF Mode = "fixed" /\ Guard = "server" /\ StaleIdentity(m.first)
  ...
  /\ UNCHANGED <<horizon, cst, rid, base, bseq, claimed, subs, createdAt, firstSend, lastSend,
                 lastClaim, sendClaim, misleading, badAck, clock, losses, tps, restarts, auths, keeps, resumes>>
...
Tick ==
  ...
  /\ UNCHANGED <<head, hseq, ownSeq, applied, rec, horizon, cst, rid, base, bseq, claimed,
                 subs, createdAt, firstSend, lastSend, lastClaim, sendClaim, lostUpdate, misleading, badAck,
...
\* R1: the "outcome unknown" review is offered only for a request that may have left
NoSpuriousReview == cst = "unknownReview" => firstSend # NoTime
\* R2 (action): past the window anchored on the last possible send, absence never leads to a submission
NoResubmitPastWindow == [][(cst = "look" /\ cst' = "sub") => HostedTrusts]_vars

\* R3: the journal anchor never postdates a record of the current identity it must cover
AnchorCovers == rec[rid].k # "none" => (AnchorTime # NoTime /\ AnchorTime <= rec[rid].at)
