...
export function renderNpm(input: {packageName:string;binName:string} = {packageName:"superbee",binName:"superbee"}): string {
  ...
  lines.push("");
  lines.push("- In a folder made by `superbee checkout` the host is the authority. On `AUTH_REQUIRED` (exit 4), relay `details.sign_in_url` to the person, then re-run the same command; `superbee setup hosted` signs in and picks the default workspace in one step.");
  lines.push("- Run `superbee sync` once at the end of a batch of edits. Any concurrent change to one document, even to different frontmatter keys, is a conflict: read `$REFS/hosted-checkout.md`, then `sync --inspect --doc <id>` and `--resolve keep|take|revise --doc <id>`. Deleted files sync as deletes; when sync reports `deletions_held`, ask the person before running its `--accept-deletes` command, else `--restore-deletes`. A refusal that says to do something in the Superbee app is for the person: tell them, and never work around it.");
  lines.push("");
