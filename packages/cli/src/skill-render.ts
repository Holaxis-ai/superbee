...
export function renderNpm(input: {packageName:string;binName:string} = {packageName:"superbee",binName:"superbee"}): string {
  ...
  lines.push("- Run `superbee sync` once at the end of a batch of edits. Any concurrent change to one document, even to different frontmatter keys, is a conflict: read `$REFS/hosted-checkout.md`, then `sync --inspect <id>` and `--resolve keep|take|revise`. A refusal that says to do something in the Superbee app is for the person: tell them, and never work around it.");
  lines.push("");
