...
// command records the CURRENT run's tokenized URL to `~/.superbee/ui-url` (0600, dir 0700, the
...
export function uiUrlFilePath(home: string = homedir()): string {
  return join(userStateDir(home), UI_URL_FILE_NAME);
}
...
export async function writeUiUrlFile(url: string, home: string = homedir()): Promise<void> {
  try {
    await writeFileAtomic0600(userStateDir(home), UI_URL_FILE_NAME, url + "\n");
  } catch {
