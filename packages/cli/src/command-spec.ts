...
/** Pure projection seam used by production and synthetic change-budget tests. */
export function projectCommandSpec(groups: readonly CommandSpecGroup[]): {
  readonly paths: readonly string[];
  readonly commandNames: readonly string[];
  readonly leaves: readonly { readonly path: string; readonly count: number }[];
  ...
  const leaves = groups.flatMap((group) => group.commands.flatMap((row) =>
    row.leaves.map((candidate) => Object.freeze({ path: candidate.path, count: candidate.arity.count }))));
