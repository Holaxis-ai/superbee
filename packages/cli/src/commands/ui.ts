...
tokenized URL is written to ~/.superbee/ui-url (0600) for one-click re-entry — a live
...
export interface UiCliDeps {
  ...
  /** Record the current tokenized URL for one-click re-entry (default: `~/.superbee/ui-url`, 0600). Injectable so tests never touch the real home dir. */
...
async function runUi({ values, positionals }: ParsedUiArgs, deps: Partial<UiCliDeps>, entry: UiEntry): Promise<void> {
  ...
  // Record this run's tokenized URL for one-click re-entry after a restart (~/.superbee/ui-url).
    ...
    render(
      {
        ...
        auth:
          "per-run session SECRET, minted fresh each boot; the first load exchanges the URL token for an HttpOnly, SameSite=Strict cookie. The current TOKENIZED URL — a live credential while this run lasts, since it embeds the token — is written to ~/.superbee/ui-url (0600) for one-click re-entry and cleared on clean shutdown; a crash leaves only a stale URL whose token dies with this process (the secret rotates next boot)",
        help: [
