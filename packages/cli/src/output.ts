...
export function renderUsage(usage:string):string {
 ...
 return bin==='superbee'?usage:usage.replace(/\bsuperbee(?= (?:doc|init|new|bundle|catalog|index|artifact|link|list|query|delete|blobs|kind|kinds|recipe|recipes|status|serve|ui|mcp|sync|home|hook|skill|session-start|version|view|setup|promote|pull|login|whoami|logout|checkout|export|--help)(?:\s|$))/g,bin);
}
