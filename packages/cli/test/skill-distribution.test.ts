...
function readStoredZipEntries(bytes: Buffer): Map<string, Buffer> {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    assert.equal(bytes.readUInt16LE(cursor + 12), 0, "portable Skill archive central time is deterministic midnight");
    assert.equal(bytes.readUInt16LE(cursor + 14), 0x0021, "portable Skill archive central date is 1980-01-01");
    ...
    assert.equal(bytes.readUInt16LE(localOffset + 10), 0, `${name}: portable Skill archive local time is deterministic midnight`);
    assert.equal(bytes.readUInt16LE(localOffset + 12), 0x0021, `${name}: portable Skill archive local date is 1980-01-01`);
