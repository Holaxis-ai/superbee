...
test("only the sign-in commands import the hosted session module", async () => {
  ...
  async function walk(dir: string): Promise<void> {
    for (const entry of await readdir(dir, { withFileTypes: true })) {
      ...
      if (/hosted-auth\//.test(text) && path.relative(src, full) !== path.join("commands", "hosted-auth.ts")) {
