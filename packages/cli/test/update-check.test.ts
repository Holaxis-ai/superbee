...
function entry(
  ...
  return {
    name: options.name ?? "@holaxis/superbee",
    version,
...
test("selection follows exact dist-tag policy for current, forward, and rollback states", () => {
  ...
  assert.equal(forward.command, "npm install --global @holaxis/superbee@0.1.0-pre.3");
  ...
  assert.equal(rollback.command, "npm install --global @holaxis/superbee@0.1.0-pre.2");
...
test("missing and malformed registry policy fail closed with no install command", () => {
  ...
  const cases: Array<[string, unknown, "tag_missing" | "malformed"]> = [
    ["missing dist-tags", { name: "@holaxis/superbee", versions: base.versions }, "tag_missing"],
...
test("network constants pin the public privacy and resource boundary", () => {
  assert.equal(UPDATE_CHECK_ENDPOINT, "https://registry.npmjs.org/%40holaxis%2Fsuperbee");
