// QA child (copied from the PR 295 round-2 probes): one `superbee sync` in its own process that SIGKILLs itself just before its Nth
...
const cfg = JSON.parse(process.env.QA_CHILD!) as { home: string; cwd: string; folder: string; bridge: string; token: string; killAt: number; argv?: string[] };
let step = 0;
...
let code = 0;
try {
  await sync(["--dir", cfg.folder, ...(cfg.argv ?? [])], {
    stdout: () => {},
    auth,
    cwd: cfg.cwd,
    fetch: bridged,
