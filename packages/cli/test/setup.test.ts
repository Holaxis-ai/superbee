...
test("setup validates state leaves and delegates execution to the calling agent", async () => {
  ...
  await assert.rejects(setup(["extra"], deps(() => {})), (error: unknown) => error instanceof CliError && error.code === "USAGE");
  let help = "";
...
test("setup executes the product-owned state recovery leaves without host inference", async () => {
  for (const operation of ["harden-state", "quarantine-state"] as const) {
    ...
    await assert.rejects(setup([operation, "--scope", "user"], injected), (error: unknown) => error instanceof CliError && error.code === "USAGE");
  }
});
...
test("setup projects and executes the one-shot state migration without host inference", async () => {
  let output = "";
  const injected = deps((text) => { output += text; });
  injected.inspectState = async () => ({ state: "migratable", reason: "validated legacy operational state is ready to migrate", records: 1 });
  await setup(["--host", "codex", "--json"], injected);
  const plan = JSON.parse(output) as { setup: { next: { command: string[]; mutates: boolean; approval: { required: boolean } } } };
  assert.deepEqual(plan.setup.next.command, ["superbee", "setup", "migrate-state"]);
  assert.equal(plan.setup.next.mutates, true);
  assert.equal(plan.setup.next.approval.required, true);
