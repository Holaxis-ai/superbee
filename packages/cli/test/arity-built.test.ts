...
function leafCases(ctx: FixtureContext): Record<PublicLeafId, LeafCase> {
  ...
  return {
    ...
    mcpInstall: simple(CLI_LEAVES.mcpInstall, ["mcp", "install"], []),
    mcpStatus: simple(CLI_LEAVES.mcpStatus, ["mcp", "status"], []),
    mcpUninstall: simple(CLI_LEAVES.mcpUninstall, ["mcp", "uninstall"], []),
    ...
    export: simple(CLI_LEAVES.export, ["export"], ["team.knowledge"], ["--host", "http://127.0.0.1:9", "--to", "exported"]),
