...
test("built init rejects surplus before creating its target", () => {
  ...
  const result = spawnSync(process.execPath, ["packages/superbee/dist/superbee.mjs", "init", "unexpected", "--dir", target, "--recipe", "none"], { cwd: join(import.meta.dirname, "../../.."), encoding: "utf8" });
