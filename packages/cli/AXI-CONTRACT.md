...
| --- | --- | --- | --- |
| AXI-01 | Ordinary output is structured TOON with an explicit JSON hatch; raw/help channels are declared exceptions. | `packages/cli-runtime/src/commands/setup.ts::Emit compact JSON instead of TOON` | `packages/cli-runtime/test/setup.test.ts::setup defaults to TOON rather than JSON` |
| AXI-02 | Lists use a uniform minimal schema, count matches, and explicit field projection; `link show` includes outbound and derived backlink totals inline. | `packages/cli-runtime/src/commands/list.ts::default schema is` `packages/cli-runtime/src/commands/link.ts::outbound_count/backlink_count always report the true` | `packages/cli-runtime/test/list.test.ts::list (unscoped): stays the minimal` `packages/cli-runtime/test/link.test.ts::link show --limit caps the outbound/backlink lists; counts stay the true totals (A5)` `packages/cli-runtime/test/link.test.ts::link show: backlink rows carry the citing link's text` |
| AXI-03 | Detail shows all fields, bounds body preview, and points to a stdout-safe byte channel. | `packages/cli-runtime/src/commands/doc/common.ts::truncates a large body (pointing at --out)` | `packages/cli-runtime/test/doc.test.ts::body truncation + --out byte-channel pointer` |
| AXI-04 | Home computes whole-bundle orientation rather than requiring client-side joins. | `packages/cli-runtime/src/commands/home.ts::render the local orientation view` | `packages/cli-runtime/test/home.test.ts::A1.1 dashboard: bundle present, docs>0` |
| AXI-05 | Empty/unreadable/offline states are explicit and distinct, and a truncated body preview identifies itself instead of passing as the complete body. | `packages/cli-runtime/src/commands/list.ts::result reports` `packages/cli-runtime/src/body-replace-guards.ts::The token every TRUNCATED body preview carries INSIDE its own value` | `packages/cli-runtime/test/list.test.ts::definitive empty state` `packages/cli-runtime/test/doc.test.ts::a truncated body is named body_preview` |
| AXI-06 | Ordinary errors are structured TOON on stdout with the capped exit taxonomy; raw `doc read --out -` and MCP JSON-RPC reserve stdout and route errors to stderr. | `packages/cli-runtime/src/output.ts::Errors are ALWAYS TOON regardless of --json` `packages/cli-runtime/src/errors.ts::The 0/1/2/4/5/6 exit taxonomy is PRESERVED` `packages/cli-runtime/src/commands/doc/read.ts::makes the channel invariant unconditional` `packages/cli-runtime/src/commands/mcp.ts::must be routed once to stderr` | `packages/cli-runtime/test/arity-built.test.ts::must keep its reserved non-error channel byte-clean` `packages/cli-runtime/test/error-boundary.test.ts::error matrix: a CliError instance passes through` `packages/cli-runtime/test/doc-cli-integration.test.ts::built CLI: raw doc-read channels route early missing-id and unknown-option envelopes only to stderr` `packages/cli-runtime/test/mcp.test.ts::mcp routes every pre-initialize failure to stderr and marks it handled` `packages/cli-runtime/test/mcp-stdio.test.ts::built npm CLI keeps MCP stdout byte-empty for usage and bundle startup failures` |
| AXI-07 | Next steps are small, scoped, runnable; hooks cover all hosts with exact ownership. | `packages/cli-runtime/src/commands/hook.ts::Claude Code, Codex, AND OpenCode` | `packages/cli-runtime/test/session-start.test.ts::into all three runtimes; status/uninstall agree` |
| AXI-08 | Bare invocation renders live orientation before the manual. | `packages/cli-runtime/src/commands/home.ts::superbee home — render the local orientation view` | `packages/cli-runtime/test/home.test.ts::identity -> bundle -> commands` |
| AXI-09 | Repeated rows expose caps, counts, and a complete-result escape. | `packages/cli-runtime/src/commands/list.ts::0 = unlimited` | `packages/cli-runtime/test/list.test.ts::count=total, shown=cap` |
| AXI-10 | The artifact identifies itself and leaf help stays focused. | `packages/cli-runtime/src/reference.ts::for a specific command's full reference.` | `packages/cli-runtime/test/doc.test.ts::each doc verb's --help is focused` |
...
| ID | Required behavior | Help/source evidence | Behavioral proof |
| --- | --- | --- | --- |
| MUTATION | Repeating an already-satisfied mutation is a successful no-op without weakening an explicit CAS premise. | `packages/cli-runtime/src/commands/doc/common.ts::Idempotent: re-writing a doc` | `packages/cli-runtime/test/doc.test.ts::doc update: idempotent` |
