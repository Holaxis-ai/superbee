...
import { parseRecipeFiles, type RecipeFile } from "../src/recipes.js";
import { MalformedDocumentError } from "../src/document-codec.js";
...
test("parseRecipeFiles: malformed frontmatter throws the shared document-codec error at every document role", () => {
  const recipe = [REFERENCE_MANIFEST, VALID_TERM, PAGE_REGISTRY, PAGE_HTML, PAGE_REFERENCE];
  for (const malformedPath of [REFERENCE_MANIFEST.path, VALID_TERM.path, PAGE_REGISTRY.path, PAGE_REFERENCE.path]) {
    for (const bytes of ["---\nvalue: [\n---\n", "---\ntype: Unclosed\n"]) {
      const files = recipe.map((file) => file.path === malformedPath ? { ...file, bytes } : file);
      assert.throws(() => parseRecipeFiles(files, "test:frontmatter"), MalformedDocumentError);
    }
  }
});
