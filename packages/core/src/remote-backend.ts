...
// The typed rejection lives in `remote-error.ts`, shared with the heads and snapshot parsers this
// backend calls; it stays importable from here for every existing consumer.
...
export class RemoteBackend implements StorageBackend {
   ...
   * digest, is rejected rather than trusted ({@link parseHeadsAnswer} owns that admission), as
   * is a `304` to a request that sent no `ifNoneMatch`. Not part of the {@link StorageBackend}
   * seam.
   */
  async heads(options: HeadsOptions = {}): Promise<HeadsResult | null> {
    ...
    return parseHeadsAnswer(await res.json());
  }
   ...
   * `SNAPSHOT_DIGEST_MISMATCH` ({@link readSnapshotStream} owns the grammar), and re-requesting
   * is the consumer's decision. Reserved files are not part of a snapshot. Not part of the
   * {@link StorageBackend} seam.
   */
  async snapshot(): Promise<RemoteSnapshot> {
    ...
    return readSnapshotStream(res.body, { status: res.status });
  }
