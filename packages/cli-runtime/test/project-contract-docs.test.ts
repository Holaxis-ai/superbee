...
test("AXI contract has ten ordered rows, live proof anchors, and a separate idempotency invariant", () => {
  ...
  assert.deepEqual(evidenceKeys(rowsById.get("AXI-02")!), [
    "packages/cli-runtime/src/commands/list.ts::default schema is",
    "packages/cli-runtime/src/commands/link.ts::outbound_count/backlink_count always report the true",
    "packages/cli-runtime/test/list.test.ts::list (unscoped): stays the minimal",
    "packages/cli-runtime/test/link.test.ts::link show --limit caps the outbound/backlink lists; counts stay the true totals (A5)",
    "packages/cli-runtime/test/link.test.ts::link show: backlink rows carry the citing link's text",
  ]);
  assert.deepEqual(evidenceKeys(rowsById.get("AXI-06")!), [
    "packages/cli-runtime/src/output.ts::Errors are ALWAYS TOON regardless of --json",
    "packages/cli-runtime/src/errors.ts::The 0/1/2/4/5/6 exit taxonomy is PRESERVED",
    "packages/cli-runtime/src/commands/doc/read.ts::makes the channel invariant unconditional",
    "packages/cli-runtime/src/commands/mcp.ts::must be routed once to stderr",
    "packages/cli-runtime/test/arity-built.test.ts::must keep its reserved non-error channel byte-clean",
    "packages/cli-runtime/test/error-boundary.test.ts::error matrix: a CliError instance passes through",
    "packages/cli-runtime/test/doc-cli-integration.test.ts::built CLI: raw doc-read channels route early missing-id and unknown-option envelopes only to stderr",
    "packages/cli-runtime/test/mcp.test.ts::mcp routes every pre-initialize failure to stderr and marks it handled",
    "packages/cli-runtime/test/mcp-stdio.test.ts::built npm CLI keeps MCP stdout byte-empty for usage and bundle startup failures",
  ]);

  assertNamedTestContains(
    "packages/cli-runtime/test/link.test.ts",
  ...
  const arityProof = readProjectFile("packages/cli-runtime/test/arity-built.test.ts");
  ...
  const errorProof = readProjectFile("packages/cli-runtime/test/error-boundary.test.ts");
  ...
  assertNamedTestContains(
    "packages/cli-runtime/test/doc-cli-integration.test.ts",
  ...
  assertNamedTestContains(
    "packages/cli-runtime/test/mcp.test.ts",
  ...
  assertNamedTestContains(
    "packages/cli-runtime/test/mcp-stdio.test.ts",
...
test("wire contract pins the complete implemented route/method table and every proof anchor", () => {
  ...
  assertEvidenceRows(
    proofRows,
    Array.from({ length: 12 }, (_, index) => `WIRE-PROOF-${String(index + 1).padStart(2, "0")}`),
  );
  ...
  const distributionAuthority = readProjectFile("packages/cli-runtime/src/distribution-resources.ts");
...
test("mandatory agent entrypoints stay compact, routed, and safe in degraded states", () => {
  ...
  for (const safetyKernel of [
    ...
    "Agents may prepare a version-bump PR and, when explicitly asked, push the `v<version>` tag that starts a release. Agents never run `npm publish`, `npm stage approve`, `npm dist-tag`, or any other authenticated npm mutation unless specifically authorized by Mike, Brian, or another human user with release authority; the human's interactive 2FA approval of the staged bytes is the one release gate. Prereleases stage on `next`, stable versions on `latest`, chosen at stage time.",
