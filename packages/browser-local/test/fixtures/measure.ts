...
export const OPERATIONS: Record<string, string> = {
  ...
  warmQuery: `${QUERIES_PER_KIND} queries by type and ${QUERIES_PER_KIND} by tag through the runtime's query verb; browser-local lists every document's head, journal and base from one IndexedDB transaction and filters in the page, request-driven pages the authority's filtered list 50 rows at a time.`,
