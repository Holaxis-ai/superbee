...
| --- | --- | --- |
| Cold open | `bootstrap`: root index, list, `readMany` batches of 25, one journaled IndexedDB write per document, from an empty working copy. | Wire capabilities, the list in pages of 50 rows, and the first 20 documents read one by one: 24, 42, and 122 requests at the three sizes (the bundle carries three convention documents beyond its size). |
