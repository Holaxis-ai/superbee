...
- Export: only when the person asks for a copy outside hosted, `superbee export <bundle-id> --to <folder>` writes a verified local bundle (`--git` for a Git board), and `superbee export --dir <checkout> --in-place` turns a checkout into a local bundle. It never changes the hosted bundle and carries no history; read `$REFS/hosted-checkout.md` first.
...
<!-- GENERATED by packages/superbee/scripts/gen-skill.mjs — do not edit by hand. -->