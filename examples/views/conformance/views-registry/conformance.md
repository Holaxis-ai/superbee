...
reply is the expected outcome), `action.propose` and `open-page` (a registry id that must not
exist), then renders a table with the request name, a status and a one-line summary for each. The entry names its revision in `<meta name="superbee-conformance-revision">`; a host that
