...
`read-versioned`, `edges`, `render-document`, `subscribe`, `host` (an undeclared capability, so a
`FORBIDDEN` reply is the expected outcome), `action.propose` and `open-page` (a registry id that
must not exist), then renders a table with the request name, a status and a one-line summary for
each. The entry names its revision in `<meta name="superbee-conformance-revision">`; a host that
byte-copies it into its own harness records that value beside its result.
