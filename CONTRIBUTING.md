...
`npm run verify:runtime-libraries -- <core.tgz> <server.tgz> <bundle-transfer.tgz>` consumes an
already-packed set and proves synchronized restricted metadata, exact core dependencies, and a
Worker-safe external consumer whose transfer import cannot reach the Node capture subpath. It never
builds or repacks the supplied artifacts. The dedicated `libraries/v<version>` workflow is the only
automated release path for that set.
Its finalizer receives only `NPM_RUNTIME_LIBRARIES_READ_TOKEN`, a granular read-only token scoped to
the three restricted packages; build and staging jobs never receive that credential.
